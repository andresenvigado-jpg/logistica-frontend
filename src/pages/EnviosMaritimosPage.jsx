...
import { getMaritimos, createMaritimo, deleteMaritimo } from '../services/enviosService';
import { getClientes } from '../services/clientesService';
import { getPuertos }  from '../services/puertosService';
...
const emptyForm = {
  ...
  fecha_registro: today, fecha_entrega: '', id_puerto: '',
  precio_envio: '', numero_flota: '', numero_guia: '',
};
...
export default function EnviosMaritimosPage() {
  ...
  const load = async () => {
    const [e, c, p] = await Promise.all([getMaritimos(), getClientes(), getPuertos()]);
    setEnvios(e.data); setClientes(c.data); setPuertos(p.data);
  };
  useEffect(() => { load(); }, []);
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteMaritimo(id); load(); }
  ...
  return (
    <div className="page">
        ...
        <table className="table">
          <thead>
            <tr>
              ...
              <th>Precio</th><th>Desc.%</th><th>Flota</th>
          ...
          <tbody>
            {envios.map((e) => (
              <tr key={e.id_envio}>
                ...
                <td><code>{e.numero_flota}</code></td>
      ...
      {modal === 'form' && (
        <Modal title="Nuevo envío marítimo" onClose={closeModal}>
          {error && <div className="alert alert-error">{error}</div>}
          <p className="tip">💡 Si la cantidad es mayor a 10, se aplica descuento del 3%</p>
          <form onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <select name="id_puerto" value={form.id_puerto} onChange={handleChange} required>
                <option value="">-- Selecciona --</option>
                {puertos.map(p => (
                  <option key={p.id_puerto} value={p.id_puerto}>{p.nombre} - {p.ciudad}</option>
                ))}
              </select>
            </div>
            ...
            <div className="form-row">
              <div>
                <label>Nº Flota (AAA1234A)</label>
                <input name="numero_flota" value={form.numero_flota} onChange={handleChange} placeholder="ABC1234Z" maxLength={8} required />
              </div>
              <div>
